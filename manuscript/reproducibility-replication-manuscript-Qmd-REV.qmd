---
title: "Reproducibility in small-N treatment research: a tutorial through the lens of aphasiology"
...
# Write as a codebook in the data folder
writeLines(md_table1, here("data", "codebook.md"))
...
To implement the two-level model, we start by filtering the original data for the same blocked condition for participant 10, but maintain the item-level data. Then, we create the level change and slope change parameters according to equation 3. The baseline slope variable is equivalent to the probe session number. The level change variable is a categorical dummy variable with values 0 for baseline, and 1 for treatment. The slope chnage variable multiplies the number of baseline sessions (6) plus 2 by level change;[^3] this value is then subtracted from the baseline slope variable. Model coefficients are visualized in Figure 2.
