---
title: |
  Reproducibility in small-N treatment research: \
  a tutorial using examples from aphasiology \
  [preprint]
author: |
...
<!-- I also used it to load data calculated elsewhere.  -->

```{r, include = FALSE, warning = FALSE, message = FALSE}
knitr::opts_chunk$set()
library(kableExtra)
# Note requires running S1-reproducible-effect-sizes first
bayesian_es = readRDS(here::here("output", "bayesian_es.rds"))
```
...
knitr::kable(cols,
             format = "latex",
             caption = "Variables and descriptions for study data from Wambaugh et al., (2017)",
             booktabs = TRUE,
             longtable = FALSE) %>%
...
$$PMG = \frac{\bar{x}_{A_2}- \bar{x}_{A_1}}{n_{Items} - \bar{x}_{A_1}}$$ {#eq-eqn2}
...
However, if we had elected to correct for baseline trends and use $\mathrm{Tau\textrm{-}U_{A\:VS.\:B\:-\:TREND\:A}}$, we can use the `Tau_U()` function. In this case, $\mathrm{Tau\textrm{-}U_{A\:VS.\:B}}$ = 1, as there are no treatment observations equal to or less than any one baseline observation, but $\mathrm{Tau\textrm{-}U_{A\:VS.\:B\:-\:TREND\:A}}$ = 0.95 due to a small baseline trend correction.
...
```{r, echo = FALSE, fig.width = 5, fig.height = 3, include = TRUE, out.width = "80%", fig.align = "center"}
#| fig.cap: Participant 10 performance during baseline and treatment phase from Wambaugh et al., (2017). Plot annotations indicate Huitema & McKean (2000) model coefficients.
knitr::include_graphics(here("manuscript", "tables-figures", "p2.png"))
```
...
[^4]: Recommendations for convergence and fit warnings: [https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#singular-models-random-effect-variances-estimated-as-zero-or-correlations-estimated-as---1)
...
read_csv(here("manuscript", "tables-figures", "table3b.csv")) %>%
  kable(format = "latex",
             caption = "Summary of major strengths and weaknesses of each effect size measure reviewed",
             booktabs = TRUE,
             longtable = FALSE) %>% 
  column_spec(1, width = "1.25in") %>% # fix width column 1
  column_spec(2, width = "3in") %>% # fix width column 2
  column_spec(3, width = "3in") %>%# fix width column 3
