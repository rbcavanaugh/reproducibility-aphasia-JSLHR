...
tibble(
  ...
  papaja::apa_table(caption = "(ref:table1-caption)", placement = "h")
```
...
2. Check that the model can successfully re-estimate the data. The dark blue line (observed data) should be centered in the light blue area (model predictions). 

```{r}
bayesplot::ppc_check(mod3, ndraws = 200)
```
...
yrep = posterior_predict(mod3, ndraws = 500)
...
ppc_stat(y, yrep, stat = "sd")
mean(apply(yrep, 1, mean) > mean(y))
mean(apply(yrep, 1, sd) > sd(y))
mean(apply(yrep, 1, e1071::kurtosis) > e1071::kurtosis(y))
```
