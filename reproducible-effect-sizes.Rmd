...
Plotting data from participant (also Figure 1.). First, we select only
...
Note that the bias_correct argument is set to `FALSE` to match what is 
...
To calculate *d*~BR~ for all participants and conditions in the Wambaugh et al, (2017) 
...
To calculate PMG for all participants and conditions in the Wambaugh et al, (2017) 
study, we created a custom function which can be found in the R/effect-size-functions.R file.

## Tau-U

The Tau-U family of effect sizes (and a number of other non-overlap measures) can
...
Figure 2. visualizes each parameter in this model structure. The code
...
Because the model structure is pre-determined, we tried a different optimizer, which
...
Notice that there is much greater uncertainty in this contrast, and as a result
...
The summary table shows us there there are statistically reliable effects
...
```{r}
head(es_logit, 20) %>% kable(format = "latex", booktabs = TRUE) %>%
  kable_styling(position = "center")
```
...
```{r}
save(p1, p2, file = here("manuscript", "tables-plots.RData"))
```
