---
title: "Supplemental Materials Part 1"
...
# "emmeans", "brms", "tidybayes", "ggdist", "tidyverse"))
...
P10 <- df %>%
  ...
  filter(participant == "P10",
         itemType == "tx",
         condition == "blocked") %>%
  # summarize below will calculate the sum for each level of session, phase
  # and spt2017
  group_by(session, phase, spt2017) %>%
  # calculates the number of correct responses for each level in group_by
  # the .groups argument removes the grouping after summarize
...
Notice that there is much greater uncertainty in this contrast, evident by the increase in standard error, and as a result
...
```{r}
getES = function(fit, adjust = FALSE){
...
```{r}
bayesian_es = getES(mod3, adjust = TRUE)
